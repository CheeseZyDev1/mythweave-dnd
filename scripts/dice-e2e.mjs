...
try {
  ...
  const startedAttack=await combatRequest({action:"start",monsterId:monsterBody.monster.id,characterId:bestiaryCharacterId});if(startedAttack.response.status!==201||startedAttack.body.combatAction?.status!=="pending"||startedAttack.body.combatAction?.base_damage<1)throw new Error(`Timing attack did not start: ${JSON.stringify(startedAttack.body)}`);
  ...
  const combatTimer=setTimeout(()=>combatReject(new Error("Spectator did not receive resolved combat in realtime.")),10000);const resolvedAttack=await combatRequest({action:"resolve",actionId:startedAttack.body.combatAction.id});if(!resolvedAttack.response.ok||resolvedAttack.body.action?.status!=="resolved"||resolvedAttack.body.action?.grade==="miss"||resolvedAttack.body.action?.applied_damage<1)throw new Error(`Timing attack resolution failed: ${JSON.stringify(resolvedAttack.body)}`);const receivedCombat=await combatEvent;clearTimeout(combatTimer);if(receivedCombat.id!==resolvedAttack.body.action.id||receivedCombat.status!=="resolved")throw new Error("Realtime timing combat result did not match.");

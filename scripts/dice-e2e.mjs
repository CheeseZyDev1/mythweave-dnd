...
try {
  ...
  let companionCombatResolve;let companionCombatReject;const companionCombatEvent=new Promise((resolve,reject)=>{companionCombatResolve=resolve;companionCombatReject=reject;});const companionCombatChannel=guest.client.channel(`companion-combat-e2e-${suffix}`).on("postgres_changes",{event:"INSERT",schema:"public",table:"companion_combat_actions",filter:`table_id=eq.${created.tableId}`},payload=>companionCombatResolve(payload.new));await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error("Companion combat subscription timed out.")),10000);companionCombatChannel.subscribe(status=>{if(status==="SUBSCRIBED"){clearTimeout(timer);resolve();}if(status==="CHANNEL_ERROR"){clearTimeout(timer);reject(new Error("Companion combat channel error."));}});});await new Promise(resolve=>setTimeout(resolve,500));
  const companionCombat=async(cookie=host.cookie())=>{const response=await fetch(`${appUrl}/api/companions/combat`,{method:"POST",headers:{"Content-Type":"application/json",Cookie:cookie},body:JSON.stringify({companionId:summonedCompanion.body.companion.id,action:"assist",monsterId:monsterBody.monster.id})});return{response,body:await response.json()};};const stolenCompanionCombat=await companionCombat(guest.cookie());if(stolenCompanionCombat.response.status!==404)throw new Error("Spectator commanded another player's companion in combat.");const companionCombatTimer=setTimeout(()=>companionCombatReject(new Error("Guest did not receive companion combat in realtime.")),10000);const assisted=await companionCombat();if(assisted.response.status!==201||assisted.body.action?.round_number!==2||assisted.body.action?.applied_damage<3||assisted.body.companion?.stance!=="assisting")throw new Error(`Companion combat assist failed: ${JSON.stringify(assisted.body)}`);const receivedCompanionCombat=await companionCombatEvent;clearTimeout(companionCombatTimer);if(receivedCompanionCombat.id!==assisted.body.action.id)throw new Error("Realtime companion combat did not match.");const duplicateCompanionCombat=await companionCombat();if(duplicateCompanionCombat.response.status!==409||duplicateCompanionCombat.body.error!=="action_spent")throw new Error("Companion used more than one action in the same initiative round.");const companionCombatPage=await fetch(`${appUrl}/dice?table=${created.tableId}`,{headers:{Cookie:host.cookie()}});const companionCombatHtml=await companionCombatPage.text();if(!companionCombatPage.ok||!companionCombatHtml.includes("ROUND-LIMITED COMBAT")||!companionCombatHtml.includes("ช่วยโจมตี")||!companionCombatHtml.includes("คุ้มกัน"))throw new Error("Companion combat controls did not render.");
  const{data:worldBoss,error:worldBossError}=await host.client.from("world_bosses").select("*").eq("status","active").limit(1).single();if(worldBossError||!worldBoss||worldBoss.hp_current<100)throw worldBossError??new Error("Active world boss is unavailable.");let worldBossResolve;let worldBossReject;const worldBossEvent=new Promise((resolve,reject)=>{worldBossResolve=resolve;worldBossReject=reject;});const worldBossChannel=guest.client.channel(`world-boss-e2e-${suffix}`).on("postgres_changes",{event:"UPDATE",schema:"public",table:"world_bosses",filter:`id=eq.${worldBoss.id}`},payload=>worldBossResolve(payload.new));await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error("World boss subscription timed out.")),10000);worldBossChannel.subscribe(status=>{if(status==="SUBSCRIBED"){clearTimeout(timer);resolve();}if(status==="CHANNEL_ERROR"){clearTimeout(timer);reject(new Error("World boss channel error."));}});});await new Promise(resolve=>setTimeout(resolve,500));
  const worldBossStrike=async(tableId,characterId,cookie)=>{const response=await fetch(`${appUrl}/api/world-boss`,{method:"POST",headers:{"Content-Type":"application/json",Cookie:cookie},body:JSON.stringify({bossId:worldBoss.id,tableId,characterId})});return{response,body:await response.json()};};const spectatorBossStrike=await worldBossStrike(created.tableId,guestCharacterId,guest.cookie());if(spectatorBossStrike.response.status!==403||spectatorBossStrike.body.error!=="player_required")throw new Error("Spectator damaged the shared world boss.");const worldBossTimer=setTimeout(()=>worldBossReject(new Error("World boss HP did not update in realtime.")),10000);const firstBossStrike=await worldBossStrike(created.tableId,bestiaryCharacterId,host.cookie());if(firstBossStrike.response.status!==201||firstBossStrike.body.contribution?.table_id!==created.tableId||firstBossStrike.body.boss?.hp_current!==worldBoss.hp_current-firstBossStrike.body.contribution.damage)throw new Error(`First party world boss strike failed: ${JSON.stringify(firstBossStrike.body)}`);const receivedWorldBoss=await worldBossEvent;clearTimeout(worldBossTimer);if(receivedWorldBoss.id!==worldBoss.id||receivedWorldBoss.hp_current!==firstBossStrike.body.boss.hp_current)throw new Error("Realtime shared boss HP did not match.");const bossCooldown=await worldBossStrike(created.tableId,bestiaryCharacterId,host.cookie());if(bossCooldown.response.status!==409||bossCooldown.body.error!=="strike_cooldown")throw new Error("World boss strike cooldown was bypassed.");
  const secondRaidResponse=await fetch(`${appUrl}/api/dice/tables`,{method:"POST",headers:{"Content-Type":"application/json",Cookie:guest.cookie()},body:JSON.stringify({action:"create",characterId:guestCharacterId})});const secondRaid=await secondRaidResponse.json();if(secondRaidResponse.status!==201)throw new Error(`Second raid room could not be created: ${JSON.stringify(secondRaid)}`);const secondBossStrike=await worldBossStrike(secondRaid.tableId,guestCharacterId,guest.cookie());if(secondBossStrike.response.status!==201||secondBossStrike.body.contribution?.table_id!==secondRaid.tableId||secondBossStrike.body.boss?.hp_current!==firstBossStrike.body.boss.hp_current-secondBossStrike.body.contribution.damage)throw new Error("Two raid rooms did not reduce the same shared HP pool.");const{data:multiPartyContributions,error:multiPartyError}=await guest.client.from("world_boss_contributions").select("table_id").eq("boss_id",worldBoss.id).in("table_id",[created.tableId,secondRaid.tableId]);if(multiPartyError||multiPartyContributions.length!==2||new Set(multiPartyContributions.map(item=>item.table_id)).size!==2)throw multiPartyError??new Error("Multi-party contribution ledger is incomplete.");const worldBossPage=await fetch(`${appUrl}/dice?table=${created.tableId}`,{headers:{Cookie:host.cookie()}});const worldBossHtml=await worldBossPage.text();if(!worldBossPage.ok||!worldBossHtml.includes("GLOBAL RAID · SHARED HP")||!worldBossHtml.includes(worldBoss.name_th)||!worldBossHtml.includes("ร่วมโจมตี World Boss"))throw new Error("World boss shared HP panel did not render.");
  const arbitration=async(payload,cookie)=>{const response=await fetch(`${appUrl}/api/arbitration`,{method:"POST",headers:{"Content-Type":"application/json",Cookie:cookie},body:JSON.stringify(payload)});return{response,body:await response.json()};};const firstReport=await arbitration({action:"report",bossId:worldBoss.id,tableId:created.tableId,category:"damage_dispute",summary:"Damage ledger diverged during the shared phase transition.",evidence:{hp:firstBossStrike.body.boss.hp_current}},host.cookie());if(firstReport.response.status!==201||firstReport.body.case?.status!=="open"||firstReport.body.case?.report_count!==1)throw new Error(`First DM report failed: ${JSON.stringify(firstReport.body)}`);const duplicateReport=await arbitration({action:"report",bossId:worldBoss.id,tableId:created.tableId,category:"damage_dispute",summary:"Duplicate report must not increase consensus.",evidence:{}},host.cookie());if(duplicateReport.response.status!==409||duplicateReport.body.error!=="already_reported")throw new Error("One DM counted twice toward arbitration consensus.");const secondReport=await arbitration({action:"report",bossId:worldBoss.id,tableId:secondRaid.tableId,category:"damage_dispute",summary:"Second raid confirms the same phase transition discrepancy.",evidence:{hp:secondBossStrike.body.boss.hp_current}},guest.cookie());if(secondReport.response.status!==201||secondReport.body.case?.id!==firstReport.body.case.id||secondReport.body.case?.status!=="ready"||secondReport.body.case?.report_count!==2)throw new Error("Reports from two DMs were not merged into a ready arbitration case.");const guestReview=await arbitration({action:"review",caseId:firstReport.body.case.id,decision:"dismissed",notes:"Unauthorized ruling attempt."},guest.cookie());if(guestReview.response.status!==403||guestReview.body.error!=="admin_required")throw new Error("Non-admin DM ruled on an arbitration case.");const{error:grantTestAdminError}=await admin.from("site_admins").insert({user_id:users[0]});if(grantTestAdminError&&!grantTestAdminError.message.includes("duplicate"))throw grantTestAdminError;const reviewed=await arbitration({action:"review",caseId:firstReport.body.case.id,decision:"compensated",notes:"สองห้องยืนยันตรงกัน ให้ชดเชยทรัพยากรแก่ปาร์ตี้"},host.cookie());if(!reviewed.response.ok||reviewed.body.case?.status!=="compensated"||reviewed.body.case?.reviewed_by!==users[0])throw new Error(`Admin arbitration failed: ${JSON.stringify(reviewed.body)}`);const arbitrationPage=await fetch(`${appUrl}/arbitration`,{headers:{Cookie:host.cookie()}});const arbitrationHtml=await arbitrationPage.text();if(!arbitrationPage.ok||!arbitrationHtml.includes("ศาลชี้ขาดเหตุการณ์")||!arbitrationHtml.includes("ADMIN ARBITRATOR")||!arbitrationHtml.includes("ชดเชยทรัพยากร"))throw new Error("Admin arbitration console did not render the decision.");const{error:removeSecondRaidError}=await admin.from("dice_table_members").delete().eq("table_id",secondRaid.tableId).eq("user_id",users[1]);if(removeSecondRaidError)throw removeSecondRaidError;
  ...
  const{data:publicCompanionCombat,error:publicCompanionCombatError}=await outsider.from("companion_combat_actions").select("id").eq("table_id",created.tableId);if(publicCompanionCombatError||publicCompanionCombat.length!==0)throw publicCompanionCombatError??new Error("RLS exposed companion combat actions.");
  const{data:publicBosses,error:publicBossesError}=await outsider.from("world_bosses").select("id");if(publicBossesError||publicBosses.length!==0)throw publicBossesError??new Error("RLS exposed world bosses publicly.");const{data:publicBossContributions,error:publicBossContributionsError}=await outsider.from("world_boss_contributions").select("id").eq("boss_id",worldBoss.id);if(publicBossContributionsError||publicBossContributions.length!==0)throw publicBossContributionsError??new Error("RLS exposed world boss contributions publicly.");
  const{data:publicArbitration,error:publicArbitrationError}=await outsider.from("dm_arbitration_cases").select("id");if(publicArbitrationError||publicArbitration.length!==0)throw publicArbitrationError??new Error("RLS exposed arbitration cases publicly.");
  ...
  await guest.client.removeChannel(companionCombatChannel);
  await guest.client.removeChannel(worldBossChannel);

...
try {
  ...
  const questPageHtml=await questPage.text();if (!questPage.ok || !questPageHtml.includes("บันทึกภารกิจ")||!questPageHtml.includes("กระดานข่าวลือ")||!questPageHtml.includes("ยังไม่มีข่าวลือ")) throw new Error("Quest Log or hidden rumor board did not render.");
  ...
  const {data:visibleSecretTemplates,error:visibleSecretTemplatesError}=await supabase.from("quest_templates").select("id").contains("tags",["secret"]);if(visibleSecretTemplatesError||visibleSecretTemplates.length!==0)throw visibleSecretTemplatesError??new Error("Secret quest templates were visible before discovery.");
  const {data:secretChainSample,error:secretChainCountError,count:secretChains}=await admin.from("secret_rumor_chains").select("id",{count:"exact"}).limit(1);if(secretChainCountError||secretChains!==5||!secretChainSample.length)throw secretChainCountError??new Error("Five secret rumor chains were not seeded.");
  const firstRumor=await questAction("rumor",{characterId});if(firstRumor.response.status!==201||firstRumor.body.rumor?.current_step!==1||firstRumor.body.rumor?.clues?.length!==1||firstRumor.body.rumor?.quest_title_th!==null||firstRumor.body.rumor?.quest_id!==null)throw new Error(`First rumor leaked or skipped secret knowledge: ${JSON.stringify(firstRumor.body)}`);
  const secondRumor=await questAction("rumor",{characterId});if(secondRumor.response.status!==201||secondRumor.body.rumor?.id!==firstRumor.body.rumor.id||secondRumor.body.rumor?.current_step!==2||secondRumor.body.rumor?.clues?.length!==2||secondRumor.body.rumor?.quest_title_th!==null)throw new Error("Rumor chain did not advance in order.");
  const revealedRumor=await questAction("rumor",{characterId});if(revealedRumor.response.status!==201||revealedRumor.body.rumor?.status!=="revealed"||revealedRumor.body.rumor?.current_step!==3||revealedRumor.body.rumor?.clues?.length!==3||!revealedRumor.body.rumor?.quest_title_th||!revealedRumor.body.rumor?.quest_id)throw new Error(`Final clue did not reveal its quest: ${JSON.stringify(revealedRumor.body)}`);
  const {data:revealedQuest,error:revealedQuestError}=await supabase.from("character_quests").select("id,title_th,status").eq("id",revealedRumor.body.rumor.quest_id).single();if(revealedQuestError||revealedQuest.title_th!==revealedRumor.body.rumor.quest_title_th||revealedQuest.status!=="active")throw revealedQuestError??new Error("Revealed secret quest was not added to the normal Quest Log.");
  const rumorPage=await fetch(`${appUrl}/quests?character=${characterId}`,{headers:{Cookie:cookie}});const rumorPageHtml=await rumorPage.text();if(!rumorPage.ok||!rumorPageHtml.includes("SECRET REVEALED")||!rumorPageHtml.includes(revealedQuest.title_th)||!rumorPageHtml.includes(firstRumor.body.rumor.clues[0].clue_th))throw new Error("Unlocked rumor chain did not persist on the Quest page.");
  const revealedRumorIds=new Set([revealedRumor.body.rumor.id]);for(let chainIndex=0;chainIndex<4;chainIndex++){let finalStep;for(let clueIndex=0;clueIndex<3;clueIndex++)finalStep=await questAction("rumor",{characterId});if(finalStep.response.status!==201||finalStep.body.rumor?.status!=="revealed")throw new Error("A later rumor chain did not reveal after exactly three clues.");revealedRumorIds.add(finalStep.body.rumor.id);}if(revealedRumorIds.size!==5)throw new Error("Rumor discovery repeated a completed chain.");const exhaustedRumors=await questAction("rumor",{characterId});if(exhaustedRumors.response.status!==409||exhaustedRumors.body.error!=="all_rumors_discovered")throw new Error("Rumor discovery did not stop after all five chains.");
  ...
  const soloAction=async action=>{const response=await fetch(`${appUrl}/api/solo`,{method:"POST",headers:{"Content-Type":"application/json",Cookie:cookie},body:JSON.stringify({characterId,action})});return{response,body:await response.json()};};const startedSolo=await soloAction("start");if(startedSolo.response.status!==201||startedSolo.body.solo?.status!=="active"||!startedSolo.body.solo?.restrictions?.includes("no_dm"))throw new Error(`Solo mode did not start in wilderness: ${JSON.stringify(startedSolo.body)}`);
  const {error:soloEscapeError}=await admin.from("character_world_positions").update({location_id:greenhollow.id}).eq("character_id",characterId);if(!soloEscapeError?.message.includes("solo wilderness only"))throw new Error("Solo wilderness restriction was bypassed at the database trigger.");
  await admin.from("generated_items").update({created_at:"2000-01-01T00:00:00Z"}).eq("character_id",characterId);const soloForge=await fetch(`${appUrl}/api/items/generate`,{method:"POST",headers:{"Content-Type":"application/json",Cookie:cookie},body:JSON.stringify({characterId,category:"weapon"})});const soloForgeBody=await soloForge.json();if(soloForge.status!==409||soloForgeBody.error!=="solo_magic_disabled")throw new Error("Solo mode allowed a magical procedural item.");
  const soloRoom=await fetch(`${appUrl}/api/dice/tables`,{method:"POST",headers:{"Content-Type":"application/json",Cookie:cookie},body:JSON.stringify({action:"create"})});if(soloRoom.status!==409||!(await soloRoom.text()).includes("solo_mode_active"))throw new Error("Solo mode allowed creating a multiplayer room.");
  const soloDm=await fetch(`${appUrl}/api/dm/manual?tableId=00000000-0000-4000-8000-000000000001`,{headers:{Cookie:cookie}});if(soloDm.status!==409||!(await soloDm.text()).includes("solo_no_dm"))throw new Error("Solo mode allowed access to DM assistance.");
  const endedSolo=await soloAction("end");if(!endedSolo.response.ok||endedSolo.body.solo?.status!=="ended")throw new Error("Solo mode did not end cleanly.");const roomAfterSolo=await fetch(`${appUrl}/api/dice/tables`,{method:"POST",headers:{"Content-Type":"application/json",Cookie:cookie},body:JSON.stringify({action:"create"})});if(roomAfterSolo.status!==201)throw new Error("Multiplayer access did not return after ending Solo mode.");
  ...
  const {data:hiddenSolo,error:hiddenSoloError}=await outsider.from("solo_adventures").select("id").eq("character_id",characterId);if(hiddenSoloError||hiddenSolo.length!==0)throw hiddenSoloError??new Error("RLS exposed solo adventure history.");
  const {data:hiddenRumorProgress,error:hiddenRumorProgressError}=await outsider.from("character_rumor_progress").select("id").eq("character_id",characterId);if(hiddenRumorProgressError||hiddenRumorProgress.length!==0)throw hiddenRumorProgressError??new Error("RLS exposed rumor progress.");
  const {data:hiddenRumorChains,error:hiddenRumorChainsError}=await outsider.from("secret_rumor_chains").select("id");if(hiddenRumorChainsError||hiddenRumorChains.length!==0)throw hiddenRumorChainsError??new Error("RLS exposed secret rumor chains.");

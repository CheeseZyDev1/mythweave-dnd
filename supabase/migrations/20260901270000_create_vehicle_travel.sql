alter table public.character_world_positions add column world_hours_elapsed bigint not null default 0 check(world_hours_elapsed>=0);
with routes(origin_slug,destination_slug,mode,hours,cost,food)as(values
('dawnspire','greenhollow','carriage',12,75,1),('dawnspire','greenhollow','griffin',3,200,1),
('dawnspire','riverrest','carriage',8,50,1),('dawnspire','riverrest','griffin',2,150,1),
('dawnspire','saltmere','carriage',18,100,2),('dawnspire','saltmere','griffin',4,250,1),
('sunharbor','riverrest','carriage',10,60,1),('sunharbor','riverrest','griffin',3,175,1))
insert into public.world_routes(from_location_id,to_location_id,travel_mode,duration_hours,cost_copper,food_cost)
select origin.id,destination.id,routes.mode,routes.hours,routes.cost,routes.food from routes
join public.world_locations origin on origin.slug=routes.origin_slug join public.world_locations destination on destination.slug=routes.destination_slug;
...
begin
  ...
  if route.cost_copper>0 then
    ...
    insert into public.wallet_transactions(character_id,user_id,delta_copper,balance_after,reason)values(target_character_id,auth.uid(),-route.cost_copper,'ค่าเดินทาง '||case target_mode when 'carriage' then 'รถม้า' else 'กริฟฟิน' end||' ไป '||destination.name_th);
  end if;
...
returns jsonb language plpgsql security definer set search_path='' as $$begin return public.travel_character_route(target_character_id,target_location_id,'fast_travel');end;$$;
revoke all on function public.travel_character_route(uuid,bigint,text) from public;
grant execute on function public.travel_character_route(uuid,bigint,text) to authenticated;

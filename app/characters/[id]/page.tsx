...
export default async function CharacterSheetPage({ params }: Props) {
  ...
  const [{ data: wallet }, { data: transactions }, {data:statusTemplates},{data:statusEffects},{data:innateAssignment},{data:skills},{data:blessingAssignment}] = await Promise.all([
    ...
    supabase.from("character_divine_blessings").select("last_triggered_at,trigger_count,definition:divine_blessing_definitions(tier,name_th,deity_th,description_th,effect_key)").eq("character_id",id).maybeSingle(),
  ]);
  ...
  const blessingDefinition=Array.isArray(blessingAssignment?.definition)?blessingAssignment.definition[0]??null:blessingAssignment?.definition??null;
  const blessing=blessingDefinition?{...blessingDefinition,last_triggered_at:blessingAssignment?.last_triggered_at??null,trigger_count:blessingAssignment?.trigger_count??0}:null;
  return <><CharacterSheet blessing={blessing} skills={assignedSkills} innate={innate} statuses={{templates:statusTemplates??[],effects:statusEffects??[]}} wallet={{balance:wallet?.balance_copper??0,transactions:(transactions??[]) as WalletTransaction[]}} character={{
    id: character.id,
    name: character.name,
    race: character.race,

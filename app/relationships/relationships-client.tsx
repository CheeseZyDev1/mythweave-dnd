...
export function RelationshipsClient({ character, profiles, initialAffinities }: { character: { id: string; name: string }; profiles: Profile[]; initialAffinities: Affinity[] }) {
  ...
  async function interact(npcId: number, action: string) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error === "cooldown" ? "NPC คนนี้ต้องการเวลาสักครู่ก่อนคุยอีกครั้ง" : result.error === "insufficient_funds" ? "เหรียญไม่พอสำหรับของขวัญ" : "การโต้ตอบไม่สำเร็จ");
  ...
  return <main className="relationship-shell">
    ...
    <section className="relationship-grid">{profiles.map((npc) => {
      ...
      return <article className={tierClass} key={npc.id}>
        <header><div className="npc-sigil">{npc.name_en.slice(0, 1)}</div><div><small>{npc.location_th}</small><h2>{npc.name_th}</h2><span>{npc.name_en} · {npc.role_th}</span></div><b>{score >= 0 ? "+" : ""}{score}<small>{tierName}</small></b></header>
        <p>{npc.description_th}</p>
        <div className="affinity-track"><i style={{ width: `${(score + 100) / 2}%` }} /></div>
        <footer>{actions.map((action) => <button key={action.id} onClick={() => interact(npc.id, action.id)} disabled={busy !== null}><span>{action.label}</span><small>{action.delta}</small></button>)}</footer>
      </article>;
    })}</section>
  </main>;
}

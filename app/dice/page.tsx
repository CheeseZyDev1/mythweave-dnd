...
export default async function DicePage({ searchParams }: Props) {
  ...
  let awareness: {
    ...
    awareness_tier: string;
  }[] = [];
  let messengerBirds = 0;
  let messengerDispatches: MessengerDispatch[] = [];
  ...
  let characterSkills: CharacterSkill[] = [];
  let skillUses: SkillUse[] = [];
  let combatDmMoments: CombatDmMoment[] = [];
  let hasSessionRecaps = false;
  const worldBoss = (activeBoss as WorldBoss | null) ?? null;
  let worldBossContributions: WorldBossContribution[] = [];
  if (tableId) {
    ...
    if (table) {
      const [
        ...
        { data: npcData },
        { data: narrationData },
        { data: combatDmMomentData },
        { data: monsterData },
        { data: companionData },
        { data: companionCommandData },
        { data: skillUseData },
        { count: recapCount },
      ] = await Promise.all([
        supabase
        ...
        supabase
          .from("room_saves")
          .select(
            "id,table_id,slot,save_name,entry_count,round_number,created_by,created_at,updated_at",
          )
          .eq("table_id", table.id)
          .order("slot"),
        supabase
        ...
        supabase
          ...
          .limit(10),
        supabase
          .from("combat_dm_moments")
          .select("*")
          .eq("table_id", table.id)
          .order("created_at", { ascending: false })
          .limit(10),
        supabase
          .from("generated_monsters")
          .select("*")
          .eq("table_id", table.id)
          .order("created_at", { ascending: false })
          .limit(12),
        supabase
          ...
          .select(
            "id,user_id,character_id,name,stance,hp_current,hp_max,guard_points,active_table_id",
          )
          .eq("active_table_id", table.id),
        supabase
          .from("homunculus_commands")
          .select("id,companion_id,command,response_th,created_at")
          .eq("table_id", table.id)
          .order("created_at", { ascending: false })
          .limit(20),
        supabase
      ...
      members = ((memberData ?? []) as BattleMemberRow[]).map((member) => ({
        ...
        character: member.character_id
          ? {
              ...
              race: member.race ?? "human",
              character_class: member.character_class ?? "fighter",
              appearance: member.appearance ?? ({} as Appearance),
              hp_current: member.hp_current ?? 0,
              hp_max: member.hp_max ?? 1,
            }
          : null,
      }));
      const ownCharacterId = members.find(
      ...
      if (ownCharacterId) {
        const [
          ...
          supabase.rpc("get_party_awareness", {
            target_table_id: table.id,
            viewer_character_id: ownCharacterId,
          }),
          supabase
            .from("character_item_stacks")
            .select("quantity,content_items!inner(slug)")
            .eq("character_id", ownCharacterId)
            .eq("content_items.slug", "messenger-raven")
            .maybeSingle(),
          supabase
            .from("messenger_dispatches")
            .select("*")
            .eq("table_id", table.id)
            .order("created_at", { ascending: false })
            .limit(12),
          supabase.rpc("get_character_room_skills", {
            target_table_id: table.id,
            target_character_id: ownCharacterId,
          }),
        ]);
        awareness = awarenessData ?? [];
        messengerBirds = birdStack?.quantity ?? 0;
        messengerDispatches = (dispatchData ?? []) as MessengerDispatch[];
        characterSkills = (skillData ?? []) as CharacterSkill[];
      }
      ...
      companions = (companionData ?? []) as RoomHomunculus[];
      companionCommands = (companionCommandData ??
        []) as RoomHomunculusCommand[];
      ...
      if (worldBoss) {
        const { data: contributionData } = await supabase
          ...
          .order("created_at", { ascending: false })
          .limit(12);
        worldBossContributions = (contributionData ??
          []) as WorldBossContribution[];
      }
    }
  }

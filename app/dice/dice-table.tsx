...
export function DiceTable({ initialTable, initialRolls, members, currentUserId, invalidTable, initialInitiativeEntries, initialInitiativeTracker }: { initialTable: TableInfo | null; initialRolls: DiceRoll[]; members: Member[]; currentUserId: string; invalidTable: boolean; initialInitiativeEntries: InitiativeEntry[]; initialInitiativeTracker: InitiativeTracker | null }) {
  ...
  async function tableAction(action: "create" | "join") {
    ...
    setError("");
    try {
      const response = await fetch("/api/dice/tables", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, code, role: joinRole }) });
  ...
  if (!table) return <main className="dice-shell"><header className="dice-topbar"><Link href="/lobby">← กลับล็อบบี้</Link><span>MYTHWEAVE · REALTIME DICE</span><i>PHASE 1</i></header><section className="dice-gateway"><small>THE SHARED TABLE</small><h1>ทอยชะตา<br />พร้อมกัน</h1><p>สร้างห้องใหม่ในฐานะ DM แล้วส่งรหัสให้เพื่อน หรือกรอกรหัสและเลือกบทบาท ทุกผลทอยจะปรากฏพร้อมกันแบบ real-time</p><div className="dice-gateway-actions"><button onClick={() => tableAction("create")} disabled={busy}>{busy ? "กำลังเปิดห้อง…" : "สร้างห้องใหม่ · เป็น DM"}</button><span>หรือเข้าร่วมด้วยรหัส</span><div className="dice-role-picker">{[["player","Player"],["dm","DM"],["spectator","Spectator"]].map(([value,label]) => <button className={joinRole === value ? "selected" : ""} onClick={() => setJoinRole(value)} key={value}>{label}</button>)}</div><div><input aria-label="รหัสห้อง" placeholder="AB12-CD34-EF56" value={code} onChange={(event) => setCode(formatCode(event.target.value))} maxLength={14} /><button onClick={() => tableAction("join")} disabled={busy || code.length !== 14}>เข้าร่วม</button></div></div>{error && <p className="dice-error">{error}</p>}<footer><b>PRIVATE ROOM</b><span>เฉพาะผู้ที่มีรหัสและล็อกอินแล้วจึงเข้าถึงข้อมูลได้</span></footer></section></main>;

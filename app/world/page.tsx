...
export default async function WorldPage({ searchParams }: { searchParams: Promise<{ character?: string }> }) {
  ...
  const [{ data: locations }, { data: routes }, { data: journey }, { data: worldEvents },{data:discoveryRows}] = await Promise.all([
    ...
    supabase.from("character_world_discoveries").select("id,location_id,discovered_at,world_discovery_templates(title_th,description_th,category,rarity)").eq("character_id",characterId).order("discovered_at",{ascending:false}).limit(12),
  ]);
  ...
  return <WorldMap
    ...
    initialDiscoveries={(discoveryRows??[]).flatMap(row=>{const detail=Array.isArray(row.world_discovery_templates)?row.world_discovery_templates[0]:row.world_discovery_templates;return detail?[{id:row.id,location_id:row.location_id,discovered_at:row.discovered_at,...detail}]:[]})}
